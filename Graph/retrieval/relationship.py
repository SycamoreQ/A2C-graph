...
def count_density(
    ...
    """
    Calculate the density of relationships for each entity.
    Density is defined as (outgoing + incoming relationships) / (total possible connections).
    
    Returns:
        A dictionary mapping entity.title -> density score
    """
    if not entities:
        return {}
    ...
    n = len(entities)
    if n <= 1:
        return {entity.title: 0.0 for entity in entities}

    # Count degrees
    density_map: dict[str, float] = {}
    entity_names = [entity.title for entity in entities]
    ...
    for entity in entities:
        ...
        # possible connections in directed graph = (n-1) * 2 (incoming + outgoing)
        possible_connections = (n - 1) * 2
        density_map[entity.title] = degree / possible_connections
